...
-define(STREAM_H,
   ...
   io:format("either the length of each chunk or its number(==schedulers).~n")
   ...
   io:format("expected output:~n"),
   io:format("[[2,7,12,15],[1,5,9,14],[3,6,8,10],[0,4,11,13]]~n~n"),
   ...
   io:format(">stream:start_pipe(Stages, L,{processes,2}).~n"),
   io:format("expected output:~n"),
   io:format("[[1,4,25,49,81,144,196,225],[0,9,16,36,64,100,121,169]]~n~n"),
   ...
   io:format(">Stages = [Stage_One, Stage_Two].~n"),
   io:format(">stream:start_piped_farm(4, Stages, L, 3).~n"),
   io:format("expected output:~n"),
   io:format("[278,206,185,200,250,121]~n")
).
