...
-define(MAPRED_H,
   ...
   io:format("an accumulator and a list to be split into chunks.~n"),
   ...
   io:format("where X is the number of processes. the list will be "),
   io:format("split into X chunks of (mostly) the same length.~n"),
   ...
   io:format("usage example 3:~n"),
   io:format(">L = [rand:uniform(100)||_<-lists:seq(1,16)].~n"),
...
-define(MAPRED_SMART_H,
   ...
   io:format("reducer process.~nthe final results are then collected "),
...
-define(STREAM_H,
   ...
   io:format("the task performed by each worker and a stream of elements to "),
   io:format("be aggregated into chunks.~n"),
   io:format("it is possible to choose the number of worker processes and "),
   io:format("the length of each chunk.~n"),
   io:format("the emitter distributes all the chunks between the workers "),
   io:format("and the collector gathers the final output stream.~n~n"),
   io:format("usage example:~n"),
   ...
   io:format("function) and a stream of elements.~n"),
...
-define(TEST_MR_H,
   ...
   io:format("computes the function f(X,EXP)=1+(sin(X))^(100*Exp), "),
   ...
   io:format("EXP(20 by default) is the exponent to be used for creating a "),
...
-define(TEST_MR_GOOGLE_H,
   ...
   io:format("or their own test directories or both.~n~n"),
   io:format("usage example 1:~n"),
   ...
   io:format("mean and median times over 12 iterations and the speedups.~n"),
   io:format("usage example 2:~n"),
   io:format(">test_mapred_google:benchmark(<schedulers-num>, ~c<", [$"]),
   io:format("test-dir>~c).~n",[$"]),
   io:format("expected output:~n"),
   io:format("same as above.~n"),
   io:format("usage example 3:~n"),
...
-define(TEST_STREAM_H,
   ...
   io:format("added together to give a final result.~n"),
   io:format("EXP(12 by default) is the exponent to be used for creating a "),
   ...
   io:format("it is also possible for the user to specify the number of "),
   io:format("workers to use, the number of schedulers, the value of EXP "),
   io:format("and the value of Chunks_EXP.~n"),
